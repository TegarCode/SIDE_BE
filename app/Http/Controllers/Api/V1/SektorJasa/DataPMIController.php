...
use App\Services\SektorJasa\DataPMIService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;
use Illuminate\Support\Facades\Cache;

class DataPMIController extends Controller
{
    private const DEFAULT_CACHE_PARTNERS = ['USA', 'CHN', 'JPN'];

    public function __construct(protected DataPMIService $service) {}
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        if ($useCache) {
            $ttl = now()->endOfDay();
            $keyNilai = $this->buildCacheKey('jasa-data-pmi:nilai', $filters);
            $keyStats = $this->buildCacheKey('jasa-data-pmi:stats', $filters);
