...
use App\Services\SektorJasa\DemografiPMIService;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Arr;
use Illuminate\Support\Facades\Cache;

class DemografiPMIController extends Controller
{
    private const DEFAULT_CACHE_PARTNERS = ['USA', 'CHN', 'JPN'];

    public function __construct(protected DemografiPMIService $service) {}
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        if ($useCache) {
            $ttl = now()->endOfDay();
            $keyNilai = $this->buildCacheKey('jasa-demografi-pmi:nilai', $filters);
            $keyStats = $this->buildCacheKey('jasa-demografi-pmi:stats', $filters);
