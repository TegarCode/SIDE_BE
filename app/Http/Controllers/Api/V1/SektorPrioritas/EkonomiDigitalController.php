...
class EkonomiDigitalController extends Controller
{
  ...
  protected string $cacheKeyHsCode = 'side_cache:sektor-prioritas:ekonomi-digital:distinct-hscode';
  protected int $cacheDuration = 259200;
  protected int $ID_Sektor = 16;

  /** GET /api/v1/tik/hscode */
  public function hscode(Request $request): JsonResponse
  {
    ...
    $cacheKey = SideCacheKey::pairs(
      ['sektor-prioritas', 'ekonomi-digital', 'distinct-hscode'],
    ...
    return response()->json([
      'success' => true,
      'message' => 'Daftar HS Code sektor tik',
      'data'    => $rows,
    ]);
  }

  public function nilaiTIK(Request $request): JsonResponse
  {
    ...
    $cacheKey = $this->buildCacheKey('nilai-tik', $filters);
    $ttl      = now()->addDays(3);
    ...
    $data = $cacheHit
      ...
      : Cache::remember(
          $cacheKey,
          $ttl,
          fn() => $this->EconomyDigitalService->getNilaiPerdaganganTIK($filters)
        );
    ...
    return ApiResponse::success($payload, 'Nilai perdagangan tik berhasil diambil', $meta);
  }

  public function nilaiTIKProduk(Request $request): JsonResponse
  {
    $filters = $this->normalizeFilters($request);
    ...
    $data = $cacheHit
      ...
      : Cache::remember(
          $cacheKey,
          $ttl,
          fn() => $this->EconomyDigitalService->nilaiPerdaganganPerProduk($filters)
        );
    ...
    return ApiResponse::success($payload, 'Produk perdagangan tik berhasil diambil', $meta);
  }

  /* ================= Helpers ================= */

  /** Ambil semua HS code sektor tik (distinct), cache sampai end-of-day. */
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'ekonomi-digital', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'ekonomi-digital', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
