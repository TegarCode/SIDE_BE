...
class PertahananController extends Controller
{
  ...
  protected string $cacheKeyHsCode = 'side_cache:sektor-prioritas:pertahanan:distinct-hscode';
  protected int $cacheDuration = 259200;
  protected int $ID_Sektor = 15;

  /** GET /api/v1/pertahanan/hscode */
  public function hscode(Request $request): JsonResponse
  {
    ...
    $cacheKey = SideCacheKey::pairs(
      ['sektor-prioritas', 'pertahanan', 'distinct-hscode'],
    ...
    return response()->json([
      'success' => true,
      'message' => 'Daftar HS Code sektor pertahanan',
      'data'    => $rows,
    ]);
  }

  public function nilaiPertahanan(Request $request): JsonResponse
  {
    ...
    $cacheKey = $this->buildCacheKey('nilai-pertahanan', $filters);
    $ttl      = now()->addDays(3);
    ...
    return ApiResponse::success($payload, 'Nilai perdagangan pertahanan berhasil diambil', $meta);
  }

  public function nilaiPertahananProduk(Request $request): JsonResponse
  {
    $filters = $this->normalizeFilters($request);
    ...
    return ApiResponse::success($payload, 'Produk perdagangan pertahanan berhasil diambil', $meta);
  }

  /* ================= Helpers ================= */

  /** Ambil semua HS code sektor pertahanan (distinct), cache sampai end-of-day. */
  private function allHsCodes(): array
  {
    ...
    $codes = Cache::remember(
      'side_cache:sektor-prioritas:pertahanan:all-hs-codes',
      now()->addDays(3),
      function () {
        $rows = DB::connection($this->conn)
          ->table("{$this->TB_SEKTOR} as s")
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'pertahanan', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'pertahanan', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
