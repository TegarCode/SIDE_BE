...
class PertahananSummaryController extends Controller
{
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'pertahanan-summary', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'pertahanan-summary', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
  ...
  public function summaryPdf(Request $request)
  {
    try {
      ...
      $cacheKey = $this->buildCacheKey('nilai-pertahanan', $filtersNegara);
      $ttl = now()->addDays(3);
      $perNegara = Cache::remember(
        $cacheKey,
        $ttl,
        fn() => $this->SektorPrioritasService->getNilaiPerdaganganEnergi($filtersNegara)
      );
      ...
      $perProduk = Cache::remember(
        $cacheKeyProduk,
        $ttl,
        fn() => $this->SektorPrioritasService->nilaiPerdaganganPerProduk($filtersProduk)
      );
      ...
      $pdf = Pdf::loadView('exports.sektor-pertahanan-summary', [
      ...
      $filename = 'sektor-pertahanan-summary-' . now()->format('Ymd_His') . '.pdf';
      ...
      report($e);
      return ApiResponse::error('Gagal memuat ringkasan sektor pertahanan');
    }
  }
}
