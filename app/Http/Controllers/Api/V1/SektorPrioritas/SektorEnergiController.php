...
class SektorEnergiController extends Controller
{
  ...
  protected string $cacheKeyHsCode = 'side_cache:sektor-prioritas:energi:distinct-hscode';
  protected int $cacheDuration = 259200;
  protected int $ID_Sektor = 1;

  /** GET /api/v1/energi/hscode */
  public function hscode(Request $request): JsonResponse
  {
    ...
    $cacheKey = SideCacheKey::pairs(
      ['sektor-prioritas', 'energi', 'distinct-hscode'],
    ...
    return response()->json([
      'success' => true,
      'message' => 'Daftar HS Code sektor energi',
      'data'    => $rows,
    ]);
  }

  public function nilaiEnergi(Request $request): JsonResponse
  {
    ...
    $cacheKey = $this->buildCacheKey('nilai-energi', $filters);
    $ttl      = now()->addDays(3);
    ...
    return ApiResponse::success($payload, 'Nilai perdagangan energi berhasil diambil', $meta);
  }

  public function nilaiEnergiProduk(Request $request): JsonResponse
  {
    $filters = $this->normalizeFilters($request);
    ...
    return ApiResponse::success($payload, 'Produk perdagangan energi berhasil diambil', $meta);
  }

  /* ================= Helpers ================= */

  /** Ambil semua HS code sektor energi (distinct), cache sampai end-of-day. */
  private function allHsCodes(): array
  {
    ...
    $codes = Cache::remember(
      'side_cache:sektor-prioritas:energi:all-hs-codes',
      now()->addDays(3),
      function () {
        $rows = DB::connection($this->conn)
          ->table("{$this->TB_SEKTOR} as s")
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'energi', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'energi', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
