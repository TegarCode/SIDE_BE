...
class SektorEnergiSummaryController extends Controller
{
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'energi-summary', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'energi-summary', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
  ...
  public function summaryPdf(Request $request)
  {
    try {
      ...
      $cacheKey = $this->buildCacheKey('nilai-energi', $filtersNegara);
      $ttl = now()->addDays(3);
      ...
      $pdf = Pdf::loadView('exports.sektor-energi-summary', [
      ...
      $filename = 'sektor-energi-summary-' . now()->format('Ymd_His') . '.pdf';
      ...
      report($e);
      return ApiResponse::error('Gagal memuat ringkasan sektor energi');
    }
  }
}
