...
class SektorFarmasiSummaryController extends Controller
{
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'farmasi-summary', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'farmasi-summary', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
  ...
  public function summaryPdf(Request $request)
  {
    try {
      ...
      $cacheKey = $this->buildCacheKey('nilai-farmasi', $filtersNegara);
      $ttl = now()->addDays(3);
      ...
      $pdf = Pdf::loadView('exports.sektor-farmasi-summary', [
      ...
      $filename = 'sektor-kesehatan-summary-' . now()->format('Ymd_His') . '.pdf';
      ...
      report($e);
      return ApiResponse::error('Gagal memuat ringkasan sektor kesehatan');
    }
  }
}
