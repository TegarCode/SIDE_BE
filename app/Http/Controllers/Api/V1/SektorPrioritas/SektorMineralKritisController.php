...
use App\Services\SektorPrioritas\MineralKritisService;
...
class SektorMineralKritisController extends Controller
{
  public function __construct(protected MineralKritisService $MineralKritisService) {}
  ...
  protected string $cacheKeyHsCode = 'side_cache:sektor-prioritas:mineral-kritis:distinct-hscode';
  protected int $cacheDuration = 259200;
  protected int $ID_Sektor = 17;

  /** GET /api/v1/mineral-kritis/hscode */
  public function hscode(Request $request): JsonResponse
  {
    ...
    $cacheKey = SideCacheKey::pairs(
      ['sektor-prioritas', 'mineral-kritis', 'distinct-hscode'],
    ...
    return response()->json([
      'success' => true,
      'message' => 'Daftar HS Code sektor mineral-kritis',
      'data'    => $rows,
    ]);
  }

  public function nilaiMineralKritis(Request $request): JsonResponse
  {
    ...
    $cacheKey = $this->buildCacheKey('nilai-mineral-kritis', $filters);
    $ttl      = now()->addDays(3);

    $data = Cache::remember(
      $cacheKey,
      $ttl,
      fn() => $this->MineralKritisService->getNilaiPerdaganganMineralKritis($filters)
    );
    ...
    return ApiResponse::success($payload, 'Nilai perdagangan mineral-kritis berhasil diambil', $meta);
  }

  public function nilaiMineralKritisProduk(Request $request): JsonResponse
  {
    $filters = $this->normalizeFilters($request);
    ...
    $data = Cache::remember(
      $cacheKey,
      $ttl,
      fn() => $this->MineralKritisService->nilaiPerdaganganPerProduk($filters)
    );
    ...
    return ApiResponse::success($payload, 'Produk perdagangan mineral-kritis berhasil diambil', $meta);
  }

  /* ================= Helpers ================= */

  /** Ambil semua HS code sektor mineral-kritis (distinct), cache sampai end-of-day. */
  private function allHsCodes(): array
  {
    ...
    $codes = Cache::remember(
      'side_cache:sektor-prioritas:mineral-kritis:all-hs-codes',
      now()->addDays(3),
      function () {
        $rows = DB::connection($this->conn)
          ->table("{$this->TB_SEKTOR} as s")
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'mineral-kritis', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'mineral-kritis', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
