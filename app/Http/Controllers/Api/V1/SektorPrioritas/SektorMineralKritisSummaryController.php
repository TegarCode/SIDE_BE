...
class SektorMineralKritisSummaryController extends Controller
{
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'mineral-kritis-summary', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'mineral-kritis-summary', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
  ...
  public function summaryPdf(Request $request)
  {
    try {
      ...
      $cacheKey = $this->buildCacheKey('nilai-mineral-kritis', $filtersNegara);
      $ttl = now()->addDays(3);
      $perNegara = Cache::remember(
        $cacheKey,
        $ttl,
        fn() => $this->MineralKritisService->getNilaiPerdaganganMineralKritis($filtersNegara)
      );
      ...
      $perProduk = Cache::remember(
        $cacheKeyProduk,
        $ttl,
        fn() => $this->MineralKritisService->nilaiPerdaganganPerProduk($filtersProduk)
      );
      ...
      $pdf = Pdf::loadView('exports.sektor-mineral-kritis-summary', [
      ...
      $filename = 'sektor-mineral-kritis-summary-' . now()->format('Ymd_His') . '.pdf';
      ...
      report($e);
      return ApiResponse::error('Gagal memuat ringkasan sektor mineral kritis');
    }
  }
}
