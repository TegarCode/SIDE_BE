...
class SektorPanganSummaryController extends Controller
{
  ...
  private function normalizeFiltersFrom(array $input): array
  {
    ...
    $hsLevel = $input['hs'] ?? null;
    if (is_numeric($hsLevel)) {
      $filters['hs'] = max(2, min(10, (int) $hsLevel));
    }

    $rawHs = $input['hscode']
      ?? ($input['hs_list'] ?? ($input['hs_codes'] ?? ($input['hsCodes'] ?? (is_string($hsLevel) ? $hsLevel : []))));
    ...
    $komoditasIds = $this->csvToIntArray($input['komoditas_ids'] ?? ($input['komoditas'] ?? []));
    if (!empty($komoditasIds)) $filters['komoditas_ids'] = $komoditasIds;
  ...
  private function buildCacheKey(string $prefix, array $filters): string
  {
    ...
    ksort($stable);
    return SideCacheKey::pairs(['sektor-prioritas', 'pangan-summary', $prefix], $stable);
  }
  ...
  private function buildCacheKeyByOriginsDests(
    ...
    return SideCacheKey::pairs(
      ['sektor-prioritas', 'pangan-summary', $prefix],
      array_merge(
        [
          'origin' => $o ?: 'all',
          'dest' => $d ?: 'all',
  ...
  public function summaryPdf(Request $request)
  {
    try {
      ...
      if ($isAllHs) {
        $filtersNegara['hs_list'] = $allHs;
      }

      $cacheKey = $this->buildCacheKey($isAllHs ? 'nilai-pangan:ALL_HSCODE' : 'nilai-pangan', $filtersNegara);
      $ttl = now()->addDays(3);
      $perNegara = Cache::remember(
        $cacheKey,
        $ttl,
        fn() => $this->PanganService->getNilaiPerdaganganPangan($filtersNegara)
      );
      ...
      if (empty($filtersProduk['hs_list'])) $filtersProduk['hs_list'] = $this->allHsList();
      ...
      $perProduk = Cache::remember(
        $cacheKeyProduk,
        $ttl,
        fn() => $this->PanganService->nilaiPerdaganganPerProduk($filtersProduk)
      );
      ...
      $pdf = Pdf::loadView('exports.sektor-pangan-summary', [
      ...
      $filename = 'sektor-pangan-summary-' . now()->format('Ymd_His') . '.pdf';
      ...
      report($e);
      return ApiResponse::error('Gagal memuat ringkasan sektor pangan');
    }
  }
}
