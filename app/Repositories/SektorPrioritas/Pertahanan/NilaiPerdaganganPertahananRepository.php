...
namespace App\Repositories\SektorPrioritas\Pertahanan;

use Illuminate\Support\Facades\DB;

class NilaiPerdaganganPertahananRepository implements NilaiPerdaganganPertahananRepositoryInterface
{
    ...
    // protected string $TB_SEKTOR    = 'tbsektor_hilirisasi'; // TIDAK DIPAKAI
    protected string $TB_SEKTOR_HS = 'tbsektor_hscode';
    protected string $COL_DIRJEN   = 'ID_WIl_Kemlu';
    protected string $UNIT         = 'Ribu US$';

    /** ID sektor untuk Pertahanan pada tbsektor_hscode */
    protected int $SEKTOR_PERTAHANAN_ID = 15;
    ...
    public function nilaiPerdaganganPerNegara(array $filters, int $kodeSumber = 5): array
    {
        ...
        if (!$y2) return ['meta' => $this->emptyMeta($filters), 'items' => []];

        $years = $this->filterYearsInRange($availableYears, $y1, $y2);
        if (empty($years)) return ['meta' => $this->emptyMeta($filters, $availableYears, $y1, $y2), 'items' => []];
        $yLast = (int) max($years);
    ...
    // ============================================================================
    //  B. PER-PRODUK (HS) PER SEKTOR PERTAHANAN — TANPA NERACA (ekspor/impor/total)
    // ============================================================================
    public function nilaiPerdaganganPerProduk(array $filters, int $kodeSumber = 5, int $limit = 50): array
    {
        ...
        [$y1, $y2, $availableYears] = $this->resolveYears($filters, $kodeSumber);
        if (!$y2) return ['meta' => $this->emptyMeta($filters), 'sektor_produk' => []];

        $years = $this->filterYearsInRange($availableYears, $y1, $y2);
        if (empty($years)) return ['meta' => $this->emptyMeta($filters, $availableYears, $y1, $y2), 'sektor_produk' => []];
        $yLast = (int) max($years);

        $sumber = $this->getSumber($kodeSumber);
        $base   = $this->baseQuery($kodeSumber, $y1, $y2, $filters);

        // total dunia per tahun (ekspor+impor)
        $worldByYear = $this->getWorldTotalsByYear($base, $years);

        // list sektor beserta HS4 (langsung dari tbsektor_hscode untuk sektor 15)
        $sektors = $this->loadSektorPertahananWithHs();

        // map nama HS
        $allHs   = [];
        foreach ($sektors as $s) $allHs = array_merge($allHs, $s['hscodes']);
    ...
    /**
     * Ambil daftar HS untuk sektor Pertahanan langsung dari tbsektor_hscode (ID_Sektor = 15).
     * Menghasilkan satu entry sektor dengan kumpulan HS4 unik.
     */
    protected function loadSektorPertahananWithHs(): array
    {
        $conn = DB::connection($this->conn);

        $rows = $conn->table($this->TB_SEKTOR_HS)
            ->where('ID_Sektor', $this->SEKTOR_PERTAHANAN_ID)
        ...
        // Nama sektor dari baris pertama, fallback "Pertahanan"
        $sektorName = (string) ($rows->first()->Sektor ?? 'Pertahanan');

        $hs4 = [];
        foreach ($rows as $h) {
            $digits = preg_replace('/\D+/', '', (string)$h->hscode);
            if ($digits !== '' && strlen($digits) >= 4) {
                $hs4[] = substr($digits, 0, 4); // HS4
            }
        }

        $hs4 = array_values(array_unique(array_filter($hs4)));
        sort($hs4, SORT_STRING);

        return [[
            'id'      => $this->SEKTOR_PERTAHANAN_ID,
            'sektor'  => $sektorName,
            'hscodes' => $hs4,
        ]];
    }
