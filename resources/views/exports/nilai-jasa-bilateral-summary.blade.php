...
<body>
    ...
    <div class="title-block">
        <h1>EVALUASI KINERJA NILAI JASA BILATERAL INDONESIA {{ $latestYear ?? '-' }}</h1>
    ...
    <div class="stats-grid">
        <div class="stat-card">
    <div class="stat-label">Total Jasa ({{ $latestYear ?? '-' }})</div>
            <div class="stat-value">
                {{ $totalLatest !== null ? number_format($totalLatest, 0, ',', '.') . ' ' . $unit : '-' }}
            </div>
        </div>
        <div class="stat-card">
    ...
    <div class="section-title">JASA INDONESIA</div>
    <div class="section-subtitle">
        Ringkasan tren jasa dan negara mitra utama pada tahun {{ $latestYear ?? '-' }}.
    </div>

    <div class="chart-grid">
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Tren Jasa</div>
                @if (!empty($trendLineChart))
                    <img class="line-chart" src="{{ $trendLineChart }}" alt="Line chart jasa keluar">
                @else
        ...
        <div class="chart-col">
            <div class="chart-box">
                ...
                @if (!empty($top5BarChart))
                    <img class="line-chart" src="{{ $top5BarChart }}" alt="Top 5 negara tujuan jasa">
                @else
                    <div class="muted" style="font-size: 10px;">Data tidak tersedia</div>
                @endif
            </div>
    ...
    <div class="section-title" style="margin-top: 12px;">
        {{ ($partnerCount ?? 0) > 10 ? 'Tabel Top 10 Mitra Jasa Indonesia' : 'Tabel Mitra Jasa Indonesia' }}
    </div>
    <div class="muted">
        @if ($prevYear)
            Perbandingan nilai jasa tahun {{ $latestYear }} dengan tahun {{ $prevYear }}.
        @else
            Ringkasan jasa pada tahun {{ $latestYear }}.
        @endif
    </div>
    ...
    @if (!empty($perProfesiRows))
        <div class="section-title" style="margin-top: 16px;">Top Profesi Jasa</div>
        <table class="data-table">
            <thead>
                <tr>
                    <th class="col-no">No</th>
            ...
            <tbody>
                @foreach ($perProfesiRows as $i => $row)
                    <tr>
                        <td class="text-center col-no">{{ $i + 1 }}</td>
                        <td>{{ $row['nama'] }}</td>
                        <td class="text-right">{{ number_format($row['latest'], 0, ',', '.') }}</td>
