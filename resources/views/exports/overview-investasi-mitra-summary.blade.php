...
<head>
    ...
    <style>
        ...
        .stat-note { font-size: 9px; color: #64748b; margin-top: 4px; }
...
<body>
    ...
    <div class="title-block">
        <h1>RINGKASAN INVESTASI {{ $countryName }}</h1>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-label">Investasi Masuk ({{ $year ?? '-' }})</div>
            <div class="stat-value">{{ $fmtVal($inNow) }} {{ $unit }}</div>
        </div>
        <div class="stat-card">
            <div class="stat-label">Investasi Keluar ({{ $year ?? '-' }})</div>
            <div class="stat-value">{{ $fmtVal($outNow) }} {{ $unit }}</div>
        </div>
        <div class="stat-card">
    ...
    <div class="section-title">Tren Investasi</div>
    <div class="section-subtitle">
        Perkembangan investasi masuk dan keluar per tahun ({{ $unit }}).
    </div>
    <div class="chart-grid">
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Tren Investasi Masuk</div>
                @if (!empty($inboundChart))
                    <img class="line-chart" src="{{ $inboundChart }}" alt="Chart investasi masuk">
                @else
        ...
        <div class="chart-col">
            <div class="chart-box">
                ...
                @if (!empty($outboundChart))
                    <img class="line-chart" src="{{ $outboundChart }}" alt="Chart investasi keluar">
                @else
                    <div class="muted" style="font-size: 10px;">Data tidak tersedia</div>
                @endif
            </div>
    ...
    @if (!empty($tableInbound))
        <div class="section-title">Top Mitra Investasi Masuk</div>
        <table class="data-table">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($tableInbound as $i => $row)
                    @php
                        ...
                        $deltaIsNA = ($valNow == 0.0 || $valPrev == 0.0);
                    @endphp
                    <tr>
    ...
    @if (!empty($tableOutbound))
        <div class="section-title">Top Mitra Investasi Keluar</div>
        <table class="data-table">
            <thead>
                <tr>
                    <th class="col-no">No</th>
    ...
    @if (!empty($timeseries))
        ...
        <div class="section-subtitle">
            Tren investasi masuk dan keluar untuk {{ $originList }} ke {{ $destList ?: 'Semua' }}
