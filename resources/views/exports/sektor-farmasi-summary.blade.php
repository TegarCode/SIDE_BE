...
<body>
    ...
    <div class="title-block">
        <h1>RINGKASAN PERDAGANGAN INTERNASIONAL SEKTOR KESEHATAN INDONESIA</h1>
        <p>
            Perdagangan kesehatan global dan bilateral produk kesehatan ({{ $produkMeta['reporter'][0]['nama'] ?? '-' }}
            ke {{ $produkMeta['partner'][0]['nama'] ?? '-' }})
        </p>
    </div>

    <div class="summary">
        Ringkasan ini menyajikan perdagangan internasional sektor kesehatan berdasarkan daftar HS terpilih pada rentang tahun
        ...
        Selain itu, ringkasan memetakan 20 negara mitra terbesar dan 20 produk kesehatan utama pada tahun terbaru,
        berikut perbandingan dengan tahun sebelumnya untuk melihat dinamika dan konsentrasi perdagangan.
        Sub-bagian bilateral produk kesehatan menekankan arah perdagangan dari asal ke tujuan yang dipilih,
    ...
    <div class="section-subtitle">
        Tren berikut menunjukkan kinerja perdagangan kesehatan Indonesia terhadap dunia pada periode
        {{ $years ? min($years) : '-' }}-{{ $years ? max($years) : '-' }} dengan satuan {{ $unit }}.
        Ekspor dan impor menggambarkan arus barang kesehatan yang keluar dan masuk, sementara nilai perdagangan
    ...
    <div class="section-title">Perdagangan Produk Kesehatan Bilateral</div>
    <div class="section-subtitle">
        Bagian ini merangkum arus perdagangan produk kesehatan dari {{ $produkMeta['reporter'][0]['nama'] ?? '-' }}
    ...
    <div class="section-subtitle">
        ...
        sehingga terlihat arah perubahan ekspor produk kesehatan utama dari {{ $produkMeta['reporter'][0]['nama'] ?? '-' }}
        ke {{ $produkMeta['partner'][0]['nama'] ?? '-' }}.
    </div>
    <table class="data-table">
        <thead>
