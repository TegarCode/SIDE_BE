...
<body>
    ...
    <div class="title-block">
        <h1>TOP INVESTASI MITRA {{ $meta['asal'] ?? '-' }}</h1>
    ...
    @if (!empty($inboundRows))
        <div class="section-title">Investasi Masuk</div>
        <div class="section-subtitle">
            Perbandingan investasi masuk dari mitra utama pada tahun {{ $meta['inbound_latest_year'] ?? '-' }} dan {{ $meta['inbound_prev_year'] ?? '-' }} ({{ $unit }}).
        </div>
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Negara</th>
                    <th>Nilai {{ $meta['inbound_prev_year'] ?? '-' }} ({{ $unit }})</th>
                    <th>Nilai {{ $meta['inbound_latest_year'] ?? '-' }} ({{ $unit }})</th>
                    <th>Perubahan (%)</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($inboundRows as $i => $row)
                    @php
                        $y2 = (string) ($meta['inbound_latest_year'] ?? '');
                        $y1 = (string) ($meta['inbound_prev_year'] ?? '');
                        $valPrev = $y1 !== '' ? ($row[$y1] ?? null) : null;
                        $valLatest = $y2 !== '' ? ($row[$y2] ?? null) : null;
                    @endphp
                    <tr>
                        ...
                        <td class="text-right">{{ $row['persen'] === null ? '-' : number_format((float) $row['persen'], 2, ',', '.') . '%' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @if (!empty($outboundRows))
        <div class="section-title">Investasi Keluar</div>
        <div class="section-subtitle">
            Perbandingan investasi keluar ke mitra utama pada tahun {{ $meta['outbound_latest_year'] ?? '-' }} dan {{ $meta['outbound_prev_year'] ?? '-' }} ({{ $unit }}).
        </div>
        <table class="data-table">
            <thead>
                <tr>
                    <th class="col-no">No</th>
                    <th>Negara</th>
                    <th>Nilai {{ $meta['outbound_prev_year'] ?? '-' }} ({{ $unit }})</th>
                    <th>Nilai {{ $meta['outbound_latest_year'] ?? '-' }} ({{ $unit }})</th>
                    <th>Perubahan (%)</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($outboundRows as $i => $row)
                    @php
                        $y2 = (string) ($meta['outbound_latest_year'] ?? '');
                        $y1 = (string) ($meta['outbound_prev_year'] ?? '');
                        $valPrev = $y1 !== '' ? ($row[$y1] ?? null) : null;
                        $valLatest = $y2 !== '' ? ($row[$y2] ?? null) : null;
                    @endphp
                    <tr>
