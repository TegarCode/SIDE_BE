...
<body>
    ...
    <div class="title-block">
        <h1>EVALUASI KINERJA TOTAL INVESTASI MASUK INDONESIA {{ $latestYear }}</h1>
    ...
    <div class="section-title">I. KINERJA INVESTASI MASUK INTERNASIONAL</div>
    <div class="section-subtitle">
        Bagian ini menyajikan ringkasan tren investasi masuk dalam beberapa tahun terakhir serta peringkat negara asal investasi utama pada tahun {{ $latestYear }}.
    </div>

    <div class="section-title" style="margin-top: 6px;">Gambaran Umum</div>
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-label">Total Investasi Masuk ({{ $latestYear }})</div>
            <div class="stat-value">{{ number_format($totalInvestasiLatest, 0, ',', '.') }} {{ $unit }}</div>
        </div>
        <div class="stat-card">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value">{{ $prevYear ? $prevYear . ' - ' . $latestYear : $latestYear }}</div>
        </div>
    </div>
    ...
    <div class="chart-grid">
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Tren Total Investasi Masuk</div>
                @if (!empty($trendLineChart))
                    <img class="line-chart" src="{{ $trendLineChart }}" alt="Line chart tren investasi masuk">
                @else
        ...
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Top 5 Negara Asal Investasi ({{ $latestYear }})</div>
                @if (!empty($top5BarChart))
                    <img class="line-chart" src="{{ $top5BarChart }}" alt="Top 5 negara asal investasi">
                @else
                    <div class="muted" style="font-size: 10px;">Data tidak tersedia</div>
                @endif
            </div>
    ...
    <div class="section-title" style="margin-top: 8px;">Tabel Top 10 Negara Asal Investasi</div>
    <div class="muted">
        @if ($prevYear)
            Perbandingan investasi masuk tahun {{ $latestYear }} dengan tahun {{ $prevYear }} beserta perubahan absolut dan persentase.
        @else
            Ringkasan investasi masuk pada tahun {{ $latestYear }}.
        @endif
    </div>
