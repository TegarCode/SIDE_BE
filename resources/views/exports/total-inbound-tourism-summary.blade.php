...
<body>
    ...
    <div class="title-block">
        <h1>EVALUASI KINERJA TOTAL WISATAWAN MASUK INDONESIA {{ $latestYear }}</h1>
    ...
    <div class="section-title">I. KINERJA WISATAWAN MASUK INTERNASIONAL</div>
    <div class="section-subtitle">
        Bagian ini menyajikan ringkasan tren wisatawan masuk dalam beberapa tahun terakhir serta peringkat negara asal wisatawan utama pada tahun {{ $latestYear }}.
    </div>

    <div class="section-title" style="margin-top: 6px;">Gambaran Umum</div>
    <div class="stats-grid">
        <div class="stat-card">
            <div class="stat-label">Total Wisatawan Masuk ({{ $latestYear }})</div>
            <div class="stat-value">{{ number_format($totalWisatawanLatest, 0, ',', '.') }} {{ $unit }}</div>
        </div>
        <div class="stat-card">
    ...
    <div class="chart-grid">
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Tren Total Wisatawan Masuk</div>
                @if (!empty($trendLineChart))
                    <img class="line-chart" src="{{ $trendLineChart }}" alt="Line chart tren wisatawan masuk">
                @else
        ...
        <div class="chart-col">
            <div class="chart-box">
                <div class="chart-title">Top 5 Negara Asal Wisatawan ({{ $latestYear }})</div>
                @if (!empty($top5BarChart))
                    <img class="line-chart" src="{{ $top5BarChart }}" alt="Top 5 negara asal wisatawan">
                @else
                    <div class="muted" style="font-size: 10px;">Data tidak tersedia</div>
                @endif
            </div>
    ...
    <div class="section-title" style="margin-top: 8px;">Tabel Top 10 Negara Asal Wisatawan</div>
    <div class="muted">
        @if ($prevYear)
            Perbandingan wisatawan masuk tahun {{ $latestYear }} dengan tahun {{ $prevYear }} beserta perubahan absolut dan persentase.
        @else
            Ringkasan wisatawan masuk pada tahun {{ $latestYear }}.
        @endif
    </div>
